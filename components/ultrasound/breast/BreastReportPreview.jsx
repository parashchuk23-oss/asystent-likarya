...
export default function BreastReportPreview({
  overview,
  conclusion,
  recommendations,
  onOverviewChange,
  ...
  return (
    <aside className="space-y-4 lg:sticky lg:top-4">
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">Опис, висновок і рекомендації можна редагувати перед копіюванням.</p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <button type="button" onClick={() => printReportDocument({ title: 'ПРОТОКОЛ УЗД МОЛОЧНИХ ЗАЛОЗ', overview, conclusion, recommendations })} className="rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-blue-200 hover:text-blue-700">
            Друк / PDF
          </button>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <textarea value={conclusion} onChange={(event) => onConclusionChange(event.target.value)} rows={6} className={textareaClass} />
      </div>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <textarea value={recommendations} onChange={(event) => onRecommendationsChange(event.target.value)} rows={5} className={textareaClass} />
        <div className="mt-3">
          <CopyButton label="Копіювати все" value={fullText} />
        </div>
      </div>
      <ReportPrintArea
        title="ПРОТОКОЛ УЗД МОЛОЧНИХ ЗАЛОЗ"
        overview={overview}
